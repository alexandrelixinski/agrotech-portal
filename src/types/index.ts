...
export function RocaPage() {
  ...
  // trocar por: listaLotes.filter(l => l.status !== 'colhido').length
  const plantiosAtivos = totalLotes

  const areaTotalHa = useMemo(
    () => listaLotes.reduce((soma, l) => soma + (Number(l.area_hectares) || Number(l.areaHectares) || 0), 0),
    [listaLotes]
  )
  ...
  return (
    <div className="min-h-screen bg-[#040C08] text-white p-4 font-sans max-w-md mx-auto pb-24">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        <div className="bg-[#121614] p-3 rounded-lg border border-white/5 flex flex-col justify-center min-h-[75px]">
          <span className="text-2xl font-bold" style={{ color: NEON_GREEN }}>{areaTotalHa.toFixed(1)} ha</span>
